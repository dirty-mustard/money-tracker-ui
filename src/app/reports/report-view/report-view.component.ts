...
import { Transaction, Report, Filter, PieChart, PIE, LINE } from '../../shared/models';
...
export class ReportViewComponent implements OnInit, OnChanges {
  ...
  public calculateExpenses(): number {
    const expenseTransactions = this.transactions.map((t: Transaction) => t.amount).filter((amount: number) => amount < 0);
  ...
  public calculateIncome(): number {
    const incomeTransactions = this.transactions.map((t: Transaction) => t.amount).filter((amount: number) => amount > 0);
  ...
  public showPieChart(): boolean {
    return this.report.hasChart(PIE);
  }

  public showLineChart(): boolean {
    return this.report.hasChart(LINE);
  }
